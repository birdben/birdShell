...
if [ ! -f $es_base_version/$es_head_plugin ]; then
  ...
  if [ $es_base_version == '2.x' ]; then
  	curl -o $es_head_plugin https://codeload.github.com/mobz/elasticsearch-head/zip/2.x
  fi
fi

es_install_cmd="tar -zxf $es_base_version/$es_tar -C $install_path/ && tar -zxf $es_base_version/$es_ik_config_tar -C $es_home/config/ && cp $es_base_version/$es_ik_plugin $es_base_version/$es_httpclient_jar $es_base_version/$es_httpcore_jar $es_home/lib/ && cp $es_base_version/$es_config_file $es_home/config/"
es_install_plugin_cmd="cp $es_base_version/$es_head_plugin $install_path/ && cd $es_home && ./bin/plugin --install head --url file://$install_path/$es_head_plugin"
