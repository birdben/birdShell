...
logstash_home=$software_path'/logstash-indexer-'${logstash_version}
...
logstash_install_cmd="tar -zxf $logstash_tar -C $software_path/ && mv $logstash_home_before $logstash_home && mkdir -p $logstash_home/conf/ && mkdir -p $logstash_home/patterns/ && cp $logstash_config_name $logstash_home/conf/ && cp $logstash_pattern $logstash_home/patterns/ && cd $logstash_home && nohup ./bin/logstash -f conf/$logstash_config_name &"
